...
class Connector {
  ...
  setColor(light, color) {
    ...
    if (color.green < 0 || color.green > 1) {
      throw new Error(
        `Color green must be between 0 and 1 but was ${color.green}`
      )
    }
    if (color.blue < 0 || color.blue > 1) {
      throw new Error(
        `Color blue must be between 0 and 1 but was ${color.blue}`
      )
    }
    this.sendCommand(
      `set light ${light.name} rgb ${color.red} ${color.green} ${color.blue}`
    )
  }
  ...
  setInterpolation(light, interpolation = 0) {
    if (interpolation < 0 || interpolation > 1) {
      throw new Error(
        `Interpolation must be between 0 and 1 but was ${interpolation}`
      )
    }
